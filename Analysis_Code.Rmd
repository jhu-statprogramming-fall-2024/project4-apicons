...
neighbourhood_priority_levels <- income_table_for_joining %>%
  ...
  mutate(number_criteria_met = income_boolean + vehicle_boolean + grocery_table_for_joining + hfai_for_joining, priority_level = case_when(number_criteria_met == 4 ~ 'High', number_criteria_met < 4 & number_criteria_met > 2 ~ 'Medium', number_criteria_met < 3 ~ 'Low'))
...
# Getting multipolygons for each neighbourhood boundary
```{r}
library(mapbaltimore)
...
# Joining master neighbourhood data with multipolygon coordinate data from mapbaltimore package 
