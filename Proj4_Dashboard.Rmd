...
master_data <- read_csv(master_url)
```
...
master_neighbourhood_data <- read_csv("master_neighbourhood_data.csv")
master_data_with_geometry <- master_neighbourhood_data %>% 
...
leaflet(master_data_with_geometry) %>%
  ...
  addPolygons(
    ...
    popup = ~paste0("Neighborhood: ", CSA2010, "<br>Category: ", priority_level),
    label = ~CSA2010
  ) %>%
  addLegend(pal = pal, values = ~priority_level, title = "Neighbourhood Healthy Food Priority Level", position = "bottomright"
  )
```
