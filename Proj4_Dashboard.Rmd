...
```{r}
h1(strong("Categorization of Baltimore Neighborhoods"), style = "font-size:15px;")
```

This page displays maps of Baltimore that indicate the priority level of each neighborhood, both today and in 2018. The maps also include information on food accessibility for each neighborhood.

```{r}
h1(strong("Find Your Nearest Grocery Store"), style = "font-size:15px;")
```
...
```{r}
h1(strong("Demographic Characteristics of Baltimore Neighborhoods"), style = "font-size:15px;")
```
...
leaflet(master_data_with_geometry) %>%
  ...
  addPolygons(
    ...
    label = ~paste0(
      ...
      "Percent of Residents with no Vehicle: ", round(percnt_no_vehicle, 1), "%<br>",
      "Healthy Food Availability Index Score: ", hfai_category, "<br>",
